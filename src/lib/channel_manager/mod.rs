...
pub struct ChannelManager {
    ...
    nominal_hashrate_per_channel: f32,
    single_submit: bool,
...
impl ChannelManager {
    ...
    /// Requests `n_standard_channels` standard and `n_extended_channels` extended channels,
    /// each advertising this miner's per-channel hashrate scaled by
    /// `nominal_hashrate_multiplier`. The channels themselves are created with the unscaled
    /// figure once the mining server answers with `OpenMiningChannel.Success`.
    pub async fn open_channels(
        ...
        let advertised_hashrate_per_channel =
            self.nominal_hashrate_per_channel * nominal_hashrate_multiplier;

        for i in 0..n_standard_channels {
            info!(
                "Sending OpenStandardMiningChannel with nominal hashrate: {} H/s",
                format_number_with_underscores(advertised_hashrate_per_channel as u64)
            );
            let open_standard_mining_channel = OpenStandardMiningChannelOwned {
                ...
                nominal_hash_rate: advertised_hashrate_per_channel,
        ...
        for i in 0..n_extended_channels {
            ...
            let open_extended_mining_channel = OpenExtendedMiningChannelOwned {
                request_id: i as u32 + n_standard_channels as u32,
                user_identity: self
                    .user_identity
                    .clone()
                    .try_into()
